...
v1  = filter([1, -1],ts, z); % orginal acc 
v2 = filter([1, - 1],ts, pose(:,3));% new z axis acc
figure(3)
hold on 
plot(t(2:end), v2(2:end), "Color",'b', 'DisplayName','detrend');
plot(t(2:end), v1(2:end), "Color",'r', 'DisplayName','orginial');
...
hold off
% plot(t, accz, "Color",'r', 'DisplayName','raw');
% motorAngles = generateMotorData(pose, hexapod); % wont run without the
% rest of the hexapod code files
