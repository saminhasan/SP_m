...
%     time = filter(ts/2, [1, ts/2-1], time0); %#ok<NASGU>
    ...
    figure('Name',"TimeWarp ","NumberTitle","off")
    hold on
    plot(time0,time0, Color='r', DisplayName='Linear time')
    plot(time0,time, Color='g', DisplayName='Warped time')
    plot(time0, sin(2*pi*f*time0), Color='b', DisplayName='Linear Sine')
    plot(time0, sin(2*pi*f*time), Color='c', DisplayName='Warped Sine')
    plot(time0,filter([1, -2, 1], ts^2, time), Color='m', DisplayName='dtime');
    % plot(time0,filter([1, -2, 1], ts^2, sin(2*pi*f*time0)), Color='m', DisplayName='dsin');
    % plot(time0,filter([1, -2, 1], ts^2, sin(2*pi*f*time)), Color='y', DisplayName='dnonlsin');
    legend()
%     f = 1/0.75;
%     %eps = 2.6e-3;
